...
class Anomaly_Classifier(nn.Module):
    def __init__(self, input_size,num_classes):
        ...
        z1 = (z-5+2*0)/1+1
        z2 = np.floor((z1-5)/2)+1
        z3 = np.floor((z2-5)/2)+1
        z4 = np.floor((z3 - 5) / 2) + 1
        z5 = np.floor((z4 - 5) / 2) + 1
        z5 = np.int(z5)

        self.dense1 = nn.Linear(32 * z5, 32)
    ...
    def forward(self, x):
        ...
        x += residual
        x = F.relu(x)
        residual = self.maxpool(x)  # [512 32 90] 183-90
...
if __name__ == '__main__':
    ...
    input_tensor = torch.autograd.Variable(torch.rand(512, 1,800)).cuda()
    inz = np.shape(input_tensor.cpu().numpy().tolist())
    model = Anomaly_Classifier(input_channel= 1, input_sizes = inz, num_classes= 5).cuda()
